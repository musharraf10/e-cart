  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1">Crafted for comfort. Designed for life.</p>
      </div>

      <div className="flex items-center gap-2">
        <span className="text-2xl font-semibold">${product.price.toFixed(2)}</span>
        {product.originalPrice > product.price && <span className="line-through text-gray-400">${product.originalPrice.toFixed(2)}</span>}
        {discount > 0 && <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">{discount}% OFF</span>}
      </div>

      <p className={`text-sm font-medium ${stockLabel === "Out of Stock" ? "text-red-600" : stockLabel === "Low Stock" ? "text-amber-600" : "text-green-600"}`}>{stockLabel}</p>
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={add} disabled={stockLabel === "Out of Stock"} className="flex-1 rounded-full bg-gray-900 text-white py-2 text-sm font-semibold disabled:opacity-50">Add to cart</button>
        <button onClick={addWishlist} className="rounded-full border px-4 py-2 text-sm">Wishlist</button>
      </div>
