...
export async function getDashboardMetrics(req, res) {
  ...
  const [
    ...
    Review.find().sort({ createdAt: -1 }).limit(5).populate("user", "name").populate("product", "name"),
    Order.aggregate([
    ...
    Product.aggregate([
      ...
      { $unwind: "$category" },
      { $project: { _id: 0, categoryId: "$category._id", name: "$category.name", products: 1 } },
    ]),
  ]);
...
export async function adminListProducts(req, res) {
  ...
  const products = await Product.find(filters).sort({ createdAt: -1 }).populate("category", "name slug");
  res.json(products);
}
...
export async function adminBulkUpdateProducts(req, res) {
  ...
  if (action === "delete") {
    ...
    await Product.updateMany({ _id: { $in: ids } }, { $set: { isVisible: false } });
  } else if (action === "update_price") {
    await Product.updateMany({ _id: { $in: ids } }, { $set: { price: Number(value) } });
  }
...
export async function adminInventoryOverview(req, res) {
  const products = await Product.find().sort({ inventoryCount: 1 }).limit(100);
  const lowStock = products.filter((p) => p.inventoryCount > 0 && p.inventoryCount < 5);
  const outOfStock = products.filter((p) => p.inventoryCount <= 0);
  res.json({ items: products, lowStockCount: lowStock.length, outOfStockCount: outOfStock.length });
}
...
export async function adminUpdateProductStock(req, res) {
  ...
  product.inventoryCount = Number(req.body.inventoryCount ?? product.inventoryCount);
...
export async function adminListOrders(req, res) {
  ...
  const orders = await Order.find(filters).sort({ createdAt: -1 }).populate("user", "name email");
  const filtered = q
    ? orders.filter(
        (o) =>
          o._id.toString().includes(q) ||
          o.user?.name?.toLowerCase().includes(String(q).toLowerCase()) ||
          o.user?.email?.toLowerCase().includes(String(q).toLowerCase()),
      )
    : orders;
  res.json(filtered);
}
...
export async function adminListCustomers(req, res) {
  const users = await User.find({ role: "customer" }).sort({ createdAt: -1 }).select("name email mobileNumber createdAt");
...
export async function adminListDrops(req, res) {
  const drops = await Drop.find().populate("product", "name slug").sort({ launchAt: 1 });
  res.json(drops);
}
...
export async function adminListReturns(req, res) {
  const returns = await ReturnRequest.find().populate("user", "name email").populate("order", "_id total status").sort({ createdAt: -1 });
  res.json(returns);
}
...
export async function adminGetAnalytics(req, res) {
  const [salesByProduct, salesByCategory, dailyRevenue, monthlyRevenue] = await Promise.all([
    Order.aggregate([
      { $unwind: "$items" },
    ...
    Product.aggregate([
      ...
      { $project: { _id: 0, category: { $ifNull: ["$category.name", "Uncategorized"] }, products: 1 } },
    ]),
    Order.aggregate([
...
export async function adminGetNotifications(req, res) {
  const [latestOrder, lowStockItems, newUser, newReview] = await Promise.all([
    ...
    Product.find({ inventoryCount: { $gt: 0, $lt: 5 } }).sort({ inventoryCount: 1 }).limit(5),
    User.findOne({ role: "customer" }).sort({ createdAt: -1 }),
    Review.findOne().sort({ createdAt: -1 }).populate("product", "name"),
  ]);

  const notifications = [];
  if (latestOrder) notifications.push({ type: "new_order", message: `New order ${latestOrder._id.toString().slice(-6)}` });
  if (lowStockItems.length) notifications.push({ type: "low_stock", message: `${lowStockItems.length} items are low stock` });
  if (newUser) notifications.push({ type: "new_user", message: `New user signup: ${newUser.name}` });
  if (newReview) notifications.push({ type: "new_review", message: `New review on ${newReview.product?.name || "product"}` });
